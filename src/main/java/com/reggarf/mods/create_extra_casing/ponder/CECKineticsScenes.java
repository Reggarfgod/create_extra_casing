...
public class CECKineticsScenes {
	...
	public static void shaftsCanBeEncased(SceneBuilder builder, SceneBuildingUtil util) {
		...
		Selection redCasing = util.select().position(3, 1, 2);
		...
		scene.overlay().showControls(util.vector().topOf(3, 1, 2), Pointing.DOWN, 60).rightClick()
			.withItem(redCasingItem);
		scene.idle(7);
		scene.world().setBlocks(redCasing, redEncased.getDefaultState()
			.setValue(EncasedShaftBlock.AXIS, Axis.X), true);
		scene.world().setKineticSpeed(shaft, 32);
		scene.idle(10);

		BlockEntry<CECEncasedShaftBlock> cyanEncasedShaft = CECBlocks.CYAN_ENCASED_SHAFT;
		...
		scene.overlay().showControls(util.vector().topOf(1, 0, 2), Pointing.UP, 60).rightClick()
			.withItem(cyanCasingItem);
		scene.idle(7);
		scene.world().setBlocks(brass, cyanEncasedShaft.getDefaultState()
			.setValue(EncasedShaftBlock.AXIS, Axis.X), true);
		scene.world().setKineticSpeed(shaft, 32);
	...
	public static void beltsCanBeEncased(SceneBuilder builder, SceneBuildingUtil util) {
		...
		scene.overlay().showText(80)
				.text("Extra Casing can be used to decorate Mechanical Belts")
				.attachKeyFrame()
